...
import { MOCHI, PALETTE, FONTS, TONE_BG } from "./data";
import {
  MochiButton,
  Fusen,
  EyebrowChip,
...
export default function MochiHouseHomePage() {
  return (
...
// ─── HERO ─── 原画忠実：2列レイアウト / コーラル下線 / ネオブル CTA / きょうのよていカード
function Hero() {
    ...
    <section className="pt-6 md:pt-8 pb-6 md:pb-8 grid grid-cols-1 md:grid-cols-[1.25fr_1fr] gap-8 md:gap-10 items-start">
      ...
      <div>
        ...
        <div className="flex gap-3 mt-6 flex-wrap">
          <MochiButton href="/p/mochi-house/live/schedule">みにいく →</MochiButton>
          <MochiButton variant="outline">めんばーになる</MochiButton>
        </div>
      </div>

      {/* RIGHT: きょうのよてい card with yellow fusen overlay */}
      <div className="relative mt-2 md:mt-8">
        ...
        >
          ...
          <div className="mt-4 flex gap-2">
            <MochiButton size="sm" href="/p/mochi-house/live/schedule">
              しらせて ♡
            </MochiButton>
            <MochiButton size="sm" variant="outline" href="/p/mochi-house/live/schedule">
              よていぜんぶ
            </MochiButton>
          </div>
        </div>

        {/* yellow fusen overlay */}
        <Fusen
          color={PALETTE.cream}
          rotate={4}
          style={{
...
// ─── Mini cards ─── 原画下部の 3 ミニカード (おしゃべり/おもいで/おとどけもの)
function MiniCards() {
  return (
    <section className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 py-4 md:py-6">
      {MOCHI.bottomCards.map((card) => {
        const hrefMap: Record<string, string> = {
          おしゃべり: "/p/mochi-house/live",
          おもいで: "/p/mochi-house/live/archive",
          おとどけもの: "/p/mochi-house/live",
        };
        const bgMap = {
          coral: PALETTE.coral,
          lilac: PALETTE.lilac,
        ...
        return (
          <Link
            key={card.t}
            href={hrefMap[card.t] ?? "/p/mochi-house/live"}
            style={{
              background: "#fff",
              borderRadius: 18,
              border: `2.5px solid ${PALETTE.ink}`,
  ...
  return (
    <section className="py-8 md:py-12">
      <div className="flex items-baseline justify-between gap-3 flex-wrap mb-5">
        ...
        <Link
          href="/p/mochi-house/live/schedule"
          style={{
            fontSize: 13,
            color: PALETTE.accent,
            fontWeight: 900,
  ...
  return (
    <section className="py-8 md:py-12">
      <div className="flex items-baseline justify-between gap-3 flex-wrap mb-5">
        ...
        <Link
          href="/p/mochi-house/live/archive"
          style={{
            fontSize: 13,
            color: PALETTE.accent,
            fontWeight: 900,
